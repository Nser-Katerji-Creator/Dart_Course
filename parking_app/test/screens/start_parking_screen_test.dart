...
void main() {
  ...
  final testParkingSpace = ParkingSpace(
    id: 'ps1', name: 'Test Space 1', address: '123 Test St', pricePerHour: 10.0, isAvailable: true);
  final testUser = Person(personalNumber: 'user123', name: 'Test User', email: 'test@example.com');
  final testVehicles = [
    Vehicle(id: 'v1', userId: 'user123', registrationNumber: 'ABC-123', type: 'Car'),
    Vehicle(id: 'v2', userId: 'user123', registrationNumber: 'XYZ-789', type: 'Motorcycle'),
  ];
  ...
  group('StartParkingScreen Widget Tests', () {
    testWidgets('renders dropdowns with default values', (WidgetTester tester) async {
      ...
      expect(find.text('Start Parking'), findsOneWidget);
    });
    ...
    testWidgets('tapping "Start Parking" dispatches StartParking event with correct endTime', (WidgetTester tester) async {
      ...
      expect(parkingData.endTime, isNotNull);
      ...
      final expectedEndTime = parkingData.startTime.add(selectedDuration);
      // Compare DateTimes by ensuring the difference is very small (e.g., < 1 second)
      expect(parkingData.endTime!.difference(expectedEndTime).inMilliseconds.abs() < 1000, isTrue);
    });
    ...
    testWidgets('shows SnackBar and pops screen on ParkingOperationSuccess', (WidgetTester tester) async {
      ...
      whenListen(
        ...
        Stream.fromIterable([ParkingLoading(), ParkingOperationSuccess(successMessage)]),
        initialState: ParkingInitial(),
      );

      await tester.pumpWidget(createTestableWidget(
          StartParkingScreen(parkingSpace: testParkingSpace),
          customParkingBloc: localMockParkingBloc // Inject controlled mock
      ));
      await tester.pumpAndSettle();

      await tester.tap(find.text('Start Parking'));
      await tester.pumpAndSettle(const Duration(seconds: 1)); // Allow time for SnackBar and pop
